...
//
// dick sites 2016.08.15
//   REVISED 2016.08.22 to fix the extrabit logic
//
...
//  from memory is dependant onthe completion of the previous load. It avoids
...
static const int kMaxArraySize = 256 * 1024 * 1024;
...
Pair* MakeLongList(uint8* ptr, int bytesize, int bytestride, bool makelinear) {
  ...
  // extrabit = 0;    // Uncomment to effectively remove 16KB xor
  ...
  for (int i = 1; i < element_count; ++i) {
    ...
    Pair* nextptr = reinterpret_cast<Pair*>(ptr + (nextelement * bytestride));
    if (i & 1) {
      nextptr = reinterpret_cast<Pair*>(ptr + ((nextelement * bytestride) ^ extrabit));
    }
// if (i < 10) fprintf(stdout, "[%d] %lx\n", i, reinterpret_cast<uintptr_t>(nextptr));
...
void TrashTheCaches(const uint8* ptr, int bytesize) {
  ...
  if (gNeverZero == 0) {fprintf(stdout, "sum = %ld\n", sum);}
}
...
int64 LinkedTiming(uint8* ptr, int bytesize, int bytestride, bool makelinear) {
  ...
  if (gNeverZero == 0) {fprintf(stdout, "pairptr->data = %ld\n", pairptr->data);}
  ...
  for (int lgcount = 4; lgcount <= 19; ++lgcount) {
   ...
   // Repeat four times. First will take cache misses, others will not if small enough
    ...
    for (int i = 0; i < 4; ++i) {
      ...
      fprintf(stdout, "%ld ", cyclesperload);
    }
    fprintf(stdout, "\n");
  }
}

Pair* MakeMyList(uint8* ptr, int kMaxArraySize, int linesize,  int cache_totalsize)
{
  Pair* pairptr = reinterpret_cast<Pair*>(ptr);
  int element_count = 64;
  // Make sure next element is in different DRAM row than current element 
  //make sure the extrabit will not change the set index
  int extrabit = 1 << 22;

  // Fill in N-1 elements, each pointing to the next one
  for (int i = 1; i < element_count; ++i) {
  	//make sure the set index of the address is the same
    Pair* nextptr = reinterpret_cast<Pair*>(ptr + (i * cache_totalsize));
    if (i & 1) {
      nextptr = reinterpret_cast<Pair*>(ptr + ((i * cache_totalsize) ^ extrabit));
    }
    //printf("%p\n",nextptr);
    pairptr->next = nextptr;
    pairptr->data = 0;
...
void  FindCacheAssociativity(uint8* ptr, int kMaxArraySize, int linesize, int totalsize) {
  //form a list and make its items map to the same set in cache
  const Pair* pairptr = MakeMyList(ptr, kMaxArraySize, linesize, totalsize);
  //test association 2,4,8,16,32,64
  for (int lgcount = 1; lgcount <=6; ++lgcount) {
    int count =  1 << lgcount;
    // Try to force the data we will access out of the caches
    TrashTheCaches(ptr, kMaxArraySize);

   // Repeat four times. First will take cache misses, others will not if small enough
    fprintf(stdout, "lgcount[%d] load %d cache lines to set[0], giving cy/ld. Repeat.  ", lgcount,count);
    for (int i = 0; i < 4; ++i) {
      int64 cyclesperload = ScrambledLoads(pairptr, count);
      fprintf(stdout, "%ld ", cyclesperload);
    }
...
int main (int argc, const char** argv) {
  ...
  for (int lgstride = kMinLgStride; lgstride <= kMaxLgStride; ++lgstride) {
    ...
    fprintf(stdout, 
            "stride[%d] naive %ld cy/ld, linear %ld cy/ld, scrambled %ld cy/ld\n",
            stride,
  ...
  // and for L3 either 2MB (dclab-1 and -2) or 3MB (dclab-3 and -4)  
